...
public:
   // Book()=default;
    Book(const string& title="unknown ", const string& author="unknown", int year=0 )
    : title(title), author(author), year(year) {}

    void display(){
        cout<<"title :  "<<this->title<<"  author :  "<<this->author<<"  year  "<<this->year<<endl;
    }
...
int main() {
    ...
    Book copylibrary(title,author,year);
    library[i]=copylibrary;
    }
